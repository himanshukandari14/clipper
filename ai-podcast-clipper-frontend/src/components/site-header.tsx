      ...
      <div className="pointer-events-auto flex items-center justify-between px-4 py-2.5 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
        <Link href="/" className="flex items-center gap-3 pl-3 group">
          ...
          <span className="font-bold text-lg tracking-tight text-white group-hover:text-primary transition-colors">
            PodClip<span className="text-white/50">.ai</span>
          </span>
        </Link>
          ...
          {["Features"].map((item) => {
            ...
            return (
              ...
              >
                <Link
                  href={href}
                  className={`text-sm font-semibold hover:text-white hover:bg-white/5 px-5 py-2 rounded-full transition-all ${isActive ? "text-primary" : "text-zinc-300"}`}
                >
                  {item}
                </Link>
              </motion.div>
          ...
          {isLoggedIn ? (
            <Link href="/dashboard">
              <motion.button
                whileHover={{
                  scale: 1.05,
                  boxShadow: "0px 0px 30px rgba(var(--primary), 0.5)",
