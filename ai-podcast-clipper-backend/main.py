...
def process_clip(
    ...
    cut_command = (
        f"ffmpeg -i {original_video_path} -ss {start_time} -t {duration} {clip_segment_path}"
    )
    ...
    s3_client.upload_file(subtitle_output_path, "ai-podcast-clipper-himanshu", output_s3_key)
...
The transcript is as follows:\n\n"""
    ...
    def process_video(
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-himanshu", s3_key, str(video_path))
...
def main():
    ...
    payload = {"s3_key": "test1/p15min.mp4"}
